...
import logging
import warnings
from utils.util import is_list_of
import torch
...
class OptimizerConstructor:
    ...
    Example 1:
        ...
        >>> optim_builder = OptimizerConstructor(
    ...
    # check if the parameters in a given parameter group are present in any of the parameter groups in a list
    def _is_in(self, param_group, param_group_list):
        """
        Check if the parameters in a given parameter group are present in any of the parameter groups in a list.

        Args:
            param_group (dict): A dictionary containing a 'params' key with a list of parameters.
            param_group_list (list): A list of dictionaries, each containing a 'params' key with a list of parameters.

        Returns:
            bool: True if there is any overlap between the parameters in param_group and any of the parameter groups in param_group_list, False otherwise.
        """
    ...
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        is_norm = isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d,
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            if not param.requires_grad:  # skip untrainable params
                params.append(param_group)
                continue
            if bypass_duplicate and self._is_in(param_group, params):
                self.logger.warning(f'{prefix} is duplicate. It is skipped since bypass_duplicate={bypass_duplicate}')
                continue
            ...
            for key in sorted_keys:
                if key in f'{prefix}.{name}':
                    ...
                    self.logger.info(f"custom key {prefix}.{name} {param_group['lr']} {param_group['weight_decay']}")
                    break
    ...
    def __call__(self, model, optim_type):
        ...
        if not self.paramwise_cfg:
            ...
            return eval(optim_type)(**optimizer_cfg)

        # set param-wise lr and weight decay recursively
        params = []
        self.add_params(params, model)
        optimizer_cfg['params'] = params
        self.logger.info(f'len of optim params: {len(params)}')
        # save the params in the optimizer_cfg
        # torch.save(params, '/home/ywh/Downloads/params.pth')

        return eval(optim_type)(**optimizer_cfg)
    
class PEFTOptimWrapperConstructor(OptimizerConstructor):
    def __init__(self, optimizer_cfg, paramwise_cfg=None):
        # assert "keywords" in optim_wrapper_cfg
        # self.keywords = optim_wrapper_cfg.pop("keywords")
        # init the OptimizerConstructor
    ...
    # def __call__(self, model: nn.Module, optim_type):
    #     model.train()
    #     if hasattr(model, "module"):
    #         model = model.module
