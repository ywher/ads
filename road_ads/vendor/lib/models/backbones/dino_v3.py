...
from .dino_v2 import named_apply
...
class DINOv3(nn.Module):
    def __init__(
        self,
        *,
        img_size: int = 224,  # 512
        patch_size: int = 16,  # 16
        in_chans: int = 3,  # 3
        pos_embed_rope_base: float = 100.0,  # 100
        pos_embed_rope_min_period: float | None = None,
        pos_embed_rope_max_period: float | None = None,
        pos_embed_rope_normalize_coords: Literal["min", "max", "separate"] = "separate",
        pos_embed_rope_shift_coords: float | None = None,  # None
        pos_embed_rope_jitter_coords: float | None = None,  # None
        pos_embed_rope_rescale_coords: float | None = None,  # 2.0
        pos_embed_rope_dtype: str = "bf16",  # 'fp32'
        embed_dim: int = 768,  # 1024
        depth: int = 12,  # 24
        num_heads: int = 12,  # 16
        ffn_ratio: float = 4.0,  # 4096/1024=4.0
        qkv_bias: bool = True,  # True
        drop_path_rate: float = 0.0,  # 0
        layerscale_init: float | None = None,  # 1e-5
        norm_layer: str = "layernorm",
        ffn_layer: str = "mlp",  # 'mlp'
        ffn_bias: bool = True,  # True
        proj_bias: bool = True,  # True
        n_storage_tokens: int = 0,  # 4
        mask_k_bias: bool = False,  # True
    ...
    def forward_features_origin(self, x: Tensor | List[Tensor], masks: Optional[Tensor] = None) -> List[Dict[str, Tensor]]:
        if isinstance(x, torch.Tensor):
            return self.forward_features_list([x], [masks])[0]
        else:
            return self.forward_features_list(x, masks)
        
    def forward_features(self, x: Tensor, masks: Optional[Tensor] = None) -> List[Tensor]:
        """
        按 out_indices 返回 list[Tensor]，每个 Tensor 形状为 [B, C, H/patch, W/patch]
        """
        B, _, h, w = x.shape
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in self.out_indices:
                # 取 patch tokens（跳过 cls 与 storage tokens）
                patch_tokens = x[:, self.n_storage_tokens + 1 :, :]  # [B, H*W, C]
                out = (
                    patch_tokens.permute(0, 2, 1)  # [B, C, H*W]
                    .reshape(B, -1, H, W)         # [B, C, H, W]
                    .contiguous()
                )
                outs.append(out)
        return outs
